...
const Component = react.forwardRef(function DataTable(props, ref) {
    const tableEl = react.useRef(null);
    const table = react.useRef(null);
    const options = react.useRef(props.options ?? {});
    ...
    return (jsxRuntime.jsx("div", { children: jsxRuntime.jsx("table", { ref: tableEl, className: props.className ?? '', id: props.id ?? '', children: props.children ?? null }) }));
});
...
function renderJsx(cache, jsx) {
    ...
    let root = client.createRoot(div);
    root.render(jsx);
    cache.push(root);
    return div;
}
